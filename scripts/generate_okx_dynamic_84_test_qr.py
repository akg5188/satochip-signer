...
OUTPUT_STEM = "okx-dynamic-49-84-test-20260416"
INTERVAL_MS = 450
...
def main() -> int:
    ...
    for index, page in enumerate(pages, start=1):
        frame_name = f"{OUTPUT_STEM}-frame-{index:02d}.png"
...
<title>OKX 动态 BTC 49+84 测试页</title>
<style>
:root {{ color-scheme: light; --ink:#111827; --muted:#475569; --paper:#fff; --line:#d7dee8; --soft:#f6f8fb; }}
* {{ box-sizing:border-box; }}
body {{ margin:0; background:linear-gradient(180deg,#f8fafc 0%,#ffffff 32%,#eef2f7 100%); color:var(--ink); font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; }}
...
.qrwrap {{ display:grid; place-items:center; margin:18px auto 8px; }}
img {{ display:block; width:320px; height:320px; image-rendering:pixelated; background:white; border:1px solid #e2e8f0; }}
...
<main>
  <section class="card">
    <h1>OKX 动态 BTC 49+84 测试页</h1>
    <p class="note">这页用于固件前测试：<code>OKX 动态 crypto-multi-accounts</code>，包含 <code>1 个标准 EVM + 10 个 OKX EVM + BTC 49' + 84'</code>。</p>
    <p>静态整串长度: <code>{len(static_ur)}</code> 字符</p>
    <p>动态帧数: <code>{len(pages)}</code> 帧，分片上限 <code>{tp_views.WEB3_OKX_CONNECT_QR_MAX_FRAGMENT_LEN}</code>，播放间隔 <code>{INTERVAL_MS}ms</code></p>
    <p>测试助记词: <code>{MNEMONIC}</code></p>
    <p>当前地址: <code>{web3_account["address"]}</code></p>
    <div class="qrwrap">
      <img id="qr" src="{frame_names[0]}" alt="OKX dynamic BTC49+84 frame">
    </div>
    ...
    <div class="toolbar">
      ...
      <button type="button" data-size="420">420</button>
    </div>
  </section>
</main>
<script>
...
"""
    ...
    build_info = {
        ...
        "btc_paths": list(tp_views.WEB3_OKX_BTC_ACCOUNT_PATHS),
        "static_length": len(static_ur),
        "dynamic_frame_count": len(pages),
